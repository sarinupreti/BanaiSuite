...
const createProject = (id: string, name: string, location: string, client: string, budget: number, progress: number, teamUsers: User[], status: ProjectStatus = ProjectStatus.ACTIVE): Project => {
    const team = generateTeam(teamUsers);
    const clientInvoices = generateClientInvoices();
    const expenses = generateExpenses(team);
    ...
    return {
        ...
        budget,
        status,
        actualCost: expenses.reduce((sum, exp) => sum + exp.amount, 0),
        revenue: clientInvoices.filter(inv => inv.status === InvoiceStatus.PAID).reduce((sum, inv) => sum + inv.amount, 0),
        progress,
        tasks: generateTasks(20, team),
        team,
        inventory: generateInventory(),
        orders: generateOrders(team),
        expenses,
        clientInvoices,
        logs: generateLogs(team),
        equipment: [],
        documents: generateDocuments(team),
        attendance: generateAttendance(team),
        siteInstructions: [],
        consumptionHistory: [],
    };
};
...
let projects: Project[] = [
    createProject('proj1', 'Everest Residential Tower', 'Kathmandu, Nepal', 'Himalayan Builders Inc.', 50000000, 45, users.slice(0, 5), ProjectStatus.ACTIVE),
    createProject('proj2', 'Pokhara Commercial Complex', 'Pokhara, Nepal', 'Lakeside Properties', 35000000, 25, users.slice(0, 4), ProjectStatus.ACTIVE),
];
...
export const mockDataService = {
  ...
  updateProjectDetails: async (projectId: string, details: Partial<Project>) => {
    await delay(300);
    const projectIndex = projects.findIndex(p => p.id === projectId);
    if (projectIndex !== -1) {
        projects[projectIndex] = { ...projects[projectIndex], ...details };
    }
  },
  ...
  createTask: async (projectId: string, taskData: Omit<Task, 'id' | 'dependencies' | 'startDate'>) => {
      await delay(300);
  ...
  updateTask: async (projectId: string, updatedTask: Task) => {
      await delay(300);
      const project = projects.find(p => p.id === projectId);
      if (project) {
  ...
  createInventoryItem: async (projectId: string, itemData: Omit<InventoryItem, 'id' | 'quantity'>) => {
    await delay(300);
    const project = projects.find(p => p.id === projectId);
    if(project) {
        ...
        project.inventory.push(newItem);
    }
  },
  updateInventoryItem: async (projectId: string, updatedItem: InventoryItem) => {
      await delay(300);
      const project = projects.find(p => p.id === projectId);
      if(project) {
          const itemIndex = project.inventory.findIndex(i => i.id === updatedItem.id);
          if (itemIndex !== -1) {
              project.inventory[itemIndex] = { ...project.inventory[itemIndex], ...updatedItem };
          }
      }
  },
  createOrder: async (projectId: string, orderData: { items: { name: string, quantity: number, unit: string }[], requestedBy: User }) => {
    await delay(300);
    const project = projects.find(p => p.id === projectId);
    if(project) {
        ...
        project.orders.unshift(newOrder);
    }
  },
  updateOrder: async (projectId: string, updatedOrder: Order) => {
      await delay(300);
      const project = projects.find(p => p.id === projectId);
      if(project) {
          const orderIndex = project.orders.findIndex(o => o.id === updatedOrder.id);
          if (orderIndex !== -1) {
              const originalOrder = project.orders[orderIndex];
              project.orders[orderIndex] = updatedOrder;
              const currentUser = await mockAuthService.getLoggedInUser();

              // If status changed to APPROVED
              if (originalOrder.status !== OrderStatus.APPROVED && updatedOrder.status === OrderStatus.APPROVED && currentUser) {
                  updatedOrder.approvedBy = currentUser;
                  project.logs.unshift({ id: faker.string.uuid(), user: currentUser, action: 'Order Approval', timestamp: new Date().toISOString(), details: `Approved order <strong>#${updatedOrder.id.slice(0,8)}...</strong>`});
              }

              // If status changed to RECEIVED, update inventory
              if (originalOrder.status !== OrderStatus.RECEIVED && updatedOrder.status === OrderStatus.RECEIVED) {
                  updatedOrder.items.forEach(orderItem => {
                      const inventoryItem = project.inventory.find(inv => inv.name === orderItem.name);
                      if (inventoryItem) {
                          inventoryItem.quantity += orderItem.quantity;
                      }
                  });
                   if (currentUser) {
                     project.logs.unshift({ id: faker.string.uuid(), user: currentUser, action: 'Order Received', timestamp: new Date().toISOString(), details: `Marked order <strong>#${updatedOrder.id.slice(0,8)}...</strong> as Received.`});
                   }
              }

              // If invoice was added
              if (!originalOrder.invoiceUrl && updatedOrder.invoiceUrl && currentUser) {
                  project.logs.unshift({ id: faker.string.uuid(), user: currentUser, action: 'Invoice Upload', timestamp: new Date().toISOString(), details: `Uploaded invoice for order <strong>#${updatedOrder.id.slice(0,8)}...</strong>`});
              }
          }
      }
  },
